...
MeetingCopilot processes microphone and meeting audio locally. It sends only selected, bounded text context and explicitly enabled images for Grok inference. Screen context starts **Off** for each session. There is no automatic meeting recording, transcript archive or screenshot history.

Subscription OAuth is the primary connection choice; an API key is optional. This build still needs its own xAI client registration and a verified subscription inference route and entitlement. A login supported for OpenCode or Grok Build does not establish those rights for MeetingCopilot. See [OAuth evidence](oauth-evidence.md) for the precise boundary. No live subscription or direct API text/image request has been verified in this workspace.
...
| Selected text for an answer | Trusted instructions, the active question and necessary antecedents, selected profile/session context, relevant conversation, summaries and clearly identified prior AI suggestions form a bounded Responses request. | Sent to the configured xAI inference endpoint. Application requests use `store:false`; provider retention is separate. |
...
| Profiles and preferences | Explicitly entered or imported profile text and nonsecret choices are stored locally. | Versioned `preferences.json` under `~/Library/Application Support/MeetingCopilot`. Profile text is plaintext, not encrypted by this application. |
...
| API key and OAuth token set | Native Keychain items hold the optional API key and the separate rotating OAuth access/refresh/expiry/client/scope set. | The items are configured as non-synchronizing and device-only. Token contents are absent from preferences and diagnostics. |
| Model assets | Public pinned files are downloaded from Hugging Face; HTTPS object-storage redirects are permitted and every file is size/hash verified. | Stored under `Application Support/MeetingCopilot/Models`. Model downloads contain no meeting audio, transcript or screenshot payload. |
| Diagnostics | Bounded counts, timings, lifecycle state and redacted error categories/codes support troubleshooting. | A small in-memory error list and native system-log events. Production logging does not include raw PCM, transcript text, request bodies, credentials or reasoning traces. |
...
| System/application audio | ScreenCaptureKit and the declared system-audio/screen purpose strings. macOS may present Screen & System Audio Recording controls, including an audio-only grant on supported versions. | Source listing proves metadata access. Only a real audio-delivery check proves the selected source is producing captured audio. |
...
| Grok sign-in | `ASWebAuthenticationSession`, PKCE and this application's registered callback. | A valid OAuth handshake establishes a token grant; it does not establish subscription-funded text/vision inference. |
