...
| [Official OpenCode integration](https://x.ai/news/grok-opencode) | Subscription-backed OAuth is supported for the named integration. The normal Freely connection now invokes official Grok Build; the native registered-client path and API-key path remain optional. | That named OAuth registration is not reused. See [the current subscription route](oauth-evidence.md). |
