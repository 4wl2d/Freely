...
actor NativeMicrophoneCapture: MicrophoneCapturing {
    ...
    private let queue = DispatchQueue(label: "local.meetingcopilot.microphone", qos: .userInitiated)
    ...
    func start(deviceID: String?, ingress: AudioIngress) async throws {
        ...
        CopilotLog.audio.info("Microphone capture started; epoch=\(startEpoch)")
    }
    func stop() async {
        epoch &+= 1
        await stopCurrent()
    ...
    private func stopCurrent() async {
        ...
        if oldSession != nil { CopilotLog.audio.info("Microphone capture stopped") }
    }
...
actor NativeSystemAudioCapture: SystemAudioCapturing {
    ...
    private var epoch: UInt64 = 0
    private let queue = DispatchQueue(label: "local.meetingcopilot.systemaudio", qos: .userInitiated)
    ...
    func start(selection: SystemAudioSelection, ingress: AudioIngress) async throws {
        ...
        do {
            ...
            CopilotLog.audio.info("System audio capture started; epoch=\(startEpoch)")
        } catch {
    ...
    private func stopCurrent() async {
        ...
        await withCheckedContinuation { continuation in queue.async { continuation.resume() } }
        if old != nil { CopilotLog.audio.info("System audio capture stopped") }
    }
}
