...
final class ApplicationModel {
    ...
    var status: String {
        if isShuttingDown { return "Closing MeetingCopilot" }
    ...
    var subscriptionConfiguration: SubscriptionClientConfiguration {
        ...
        configuration.clientID = preferences.subscriptionClientID.isEmpty
            ? (Bundle.main.object(forInfoDictionaryKey: "MeetingCopilotOAuthClientID") as? String ?? "")
    ...
    func requestMicrophone() async {
        ...
        CopilotLog.permissions.info("Microphone authorization checked; status=\(self.microphonePermission.rawValue)")
        if microphonePermission != .authorized { showError("Microphone access is denied. Enable MeetingCopilot in Privacy & Security → Microphone.") }
    }
    func refreshDevices() {
        ...
        let task = Task { [weak self] in
            ...
            do {
                ...
                if native.domain == SCStreamErrorDomain, native.code == -3801 {
                    showError("macOS denied capture authorization (\(diagnostic)). Allow MeetingCopilot in Privacy & Security → Screen & System Audio Recording, then quit and reopen the app if macOS requests it.")
                } else {
