...
final class GenerationCoordinator {
    ...
    private let logger = Logger(subsystem: "local.meetingcopilot.app", category: "llm")
    ...
    private func run(question: QuestionState, identity: GenerationIdentity, visual: Bool, manual: Bool, detailed: Bool, speculative: Bool) async {
        ...
        do {
            ...
            CopilotLog.context.info("Context snapshot prepared; revision=\(snapshot.revision), estimated_tokens=\(snapshot.estimatedTokens)")
