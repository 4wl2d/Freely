        ...
        HStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 5) {
                HStack(spacing: 9) {
                    ...
                    Text("MeetingCopilot").font(.headline)
                }.padding(.vertical, 20)
            ...
            Section("Connect Grok") {
                ...
                if model.preferences.connectionMethod == .subscription {
                    ...
                    DisclosureGroup("Advanced subscription integration setup") {
                        TextField("Registered MeetingCopilot client ID", text: $model.preferences.subscriptionClientID)
                            .disabled(model.running || model.preparing || model.subscription.signingIn)
                        LabeledContent("Registered callback", value: "meetingcopilot://oauth/callback")
                        Text("This build has no provider-issued OAuth client registration. Use only a client registered for MeetingCopilot; another application's client ID is not a substitute.").font(.caption).foregroundStyle(.secondary)
