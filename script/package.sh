...
file "$APP/Contents/MacOS/Freely"
archive_app
if [[ -n "${FREELY_NOTARY_PROFILE:-}" ]]; then
  if [[ -z "${FREELY_SIGN_IDENTITY:-}" || "$FREELY_SIGN_IDENTITY" == - ]]; then
    echo 'Notarization requires a user-supplied Developer ID signing identity.' >&2
    exit 1
  fi
  xcrun notarytool submit "$ZIP" --keychain-profile "$FREELY_NOTARY_PROFILE" --wait
