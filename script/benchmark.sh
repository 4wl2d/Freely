...
MODE="${1:---core}"
case "$MODE" in
  --core) swift run -c release --package-path Packages/CopilotCore CopilotCoreBenchmarks --output "$TASK_ROOT/Benchmarks/results/core-replay-4h.json" ;;
  --stt-build) Benchmarks/STT/run.sh build ;;
  --stt-initial) Benchmarks/STT/run.sh initial ;;
  --stt-dual) Benchmarks/STT/run.sh dual "${2:-1200}" ;;
  --soak)
    export MEETINGCOPILOT_SOAK=1
    export MEETINGCOPILOT_SOAK_SECONDS="${2:-14400}"
