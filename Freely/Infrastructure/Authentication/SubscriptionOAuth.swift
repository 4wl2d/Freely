...
struct SubscriptionClientConfiguration: Codable, Equatable, Sendable {
    var clientID: String = ""
    var redirectURI: String = "freely://oauth/callback"
    ...
    func validate() throws {
        ...
        guard clientID.utf8.count <= 256,
              ...
              redirectURI == "freely://oauth/callback" else { throw OAuthError.invalidConfiguration }
    }
}
...
enum OAuthError: Error, LocalizedError, Sendable, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .registrationRequired: "Grok subscription connection needs an xAI OAuth registration for Freely. Add the provider-issued client ID in advanced connection settings. No other application's client identity is used."
        ...
        case .clientRejected: "xAI rejected this OAuth client. Freely needs its own registered native client with the requested access."
...
struct OAuthAuthorizationAttempt: Sendable {
    ...
    func authorizationCode(from callback: URL) throws -> String {
        guard callback.absoluteString.utf8.count <= 8_192,
              callback.scheme == "freely", callback.host == "oauth", callback.path == "/callback",
...
struct OAuthTokenStore: OAuthTokenStoring {
    let service: String
    init(service: String = "local.freely.subscription") { self.service = service }
...
final class SubscriptionAuthentication: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    init(tokens: OAuthTokenClient = OAuthTokenClient(), authorizationTimeout: Duration = .seconds(600),
         webSessionFactory: @escaping OAuthWebSessionFactory = { url, completion in
             ASWebAuthenticationSession(url: url, callbackURLScheme: "freely", completionHandler: completion)
         }) {
